...
def main():
    ...
    while True:
        ...
        if hands:
            ...
            #keyboard.press(labels[index])
            finaltext+=labels[index]
            cv2.rectangle(imgOutput, (x - offset, y - offset),
                          (x + w + offset, y + h + offset), (255, 0, 255), 4)
            cv2.rectangle(imgOutput,(20,350),(700,450),(180,180,2400),-1)
            cv2.putText(imgOutput,finaltext,(0,400),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,255),5)
            sleep(0.2)
            #cv2.putText(imgOutput, str(prediction), (20, 40), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 255, 0), 1)
        # Display
            cv2.imshow("imagecrop",imgCrop)
            cv2.imshow("imagewhite",imgWhite)
        cv2.imshow("output imahe",imgOutput)
        key=cv2.waitKey(1)
        if key == ord("s"):
            counter += 1
